...
export const InviteCard = ({ t, padding=5, gap=2, to, background, text, address, onUpdateStatus = (status) => false, size = "fullwidth", baseStyle = {} }) => {
  const _arr = text.body.split("/n");
  return (
    <Flex size={size} padding={padding} gap={gap} baseStyle={baseStyle}>
      {(!background.image) ? null : <ColorBackground color={background.color} />}
      {(!background.image) ? null : <ImageBackground src={background.image} size="cover" />}
      ...
      {
        _arr.map((t, idx) =>
          <Flex key={idx} size="fullwidth">
            <Text size={text.size} weight={2} color={text.color}>{t}</Text>
          </Flex>
        )
      } 
