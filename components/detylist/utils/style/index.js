...
const _convertKeyValPairStyle = (key, val, baseTheme, prevStyle) => {
  ...
  if (key === "size" || key === "minSize" || key === "maxSize") {
    if (val === false) { return {}; }
    if (typeof val === "string" && val.toLowerCase() === "fullwidth") {
      return _convertKeyValPairStyle(key, ["100%", "auto"], baseTheme, prevStyle);
    }

    if (typeof val === "string" && val.toLowerCase() === "full") {
      return _convertKeyValPairStyle(key, ["100%", "100%"], baseTheme, prevStyle);
    }
  ...
  if (key === "background") {
    ...
    const _convertVal = _convertColor(_val, baseTheme.colorPalette, baseTheme.color);
    if (_convertVal != _val || _convertVal[0] === "#" ||
        _convertVal.indexOf("hsl") === 0 || _convertVal.indexOf("hsla") === 0 ||
        _convertVal.indexOf("rgb") === 0 || _convertVal.indexOf("rgba") === 0)
    { return { background: _convertVal }; }
