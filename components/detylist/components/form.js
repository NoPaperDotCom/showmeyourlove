...
const BaseInput = React.forwardRef(({
  ...
  const _baseInputStyle = { color, backgroundColor: "", border: "", outline: "", appearance: false, size: "100%", ...baseStyle };
  ...
  return (
    ...
    >
      <PureComponent
        ...
        placeholder={((label && !floatLabel && !(_isValid instanceof Error)) || eventName === "focus") ? "" : _placeholder}
        onBlur={_onBlur}
        {...props}
      />
      {children}
...
const _ColorInput = React.forwardRef(({
  children,
  ...props
}, ref) => <BaseInput ref={ref} type="color" baseStyle={{ size: ["100%", 2.5] }} {...props}>{children}</BaseInput>);

_ColorInput.displayName = "ColorInput";
export const ColorInput = _ColorInput;

const _TimeInput = React.forwardRef(({
  children,
  ...props
}, ref) => <BaseInput ref={ref} type="time" {...props}>{children}</BaseInput>);
...
const _dialCodes =  [...Array(1000).keys()].map(i => `+${i + 1}`);
const _TelInput = React.forwardRef(({
  label = true, // true or false
  floatLabel = false, // true or false
  placeholder = "", // "" 
  validation = true, // true or (val) => return true / "error" / new Error
  ...
  onChange = false,
  onBlur = false,
  value = false, //"+852 12345678",
  children,
  ...props
}, ref) => {
  const _newRef = React.useRef(null);
  const _zipCodeRef = React.useRef(null);
  const _dataListId = React.useId();
  const _phoneRef = React.useRef(null);

  React.useEffect(() => {
    const _arr = (value) ? value.split(" ") : ["+852", ""];
    _zipCodeRef.current.value = (_dialCodes.indexOf(_arr[0]) !== -1) ? _arr[0] : "+852";
    _phoneRef.current.value = (_arr.length <= 1) ? "" : _arr[1];    
    
    const _el = (ref) ? ref.current : _newRef.current;
    _el.value = `${_zipCodeRef.current.value} ${_phoneRef.current.value}`;
    return () => true;
  }, [value]);

  const { eventName } = useEventListener([_zipCodeRef, _phoneRef], "focus,blur");
  const [_isValid, _setIsValid] = React.useState(true);
  useMethod((ref) ? ref : _newRef, "reset", (value) => {
    const _arr = (value) ? value.split(" ") : ["+852", ""];
    _zipCodeRef.current.value = (_dialCodes.indexOf(_arr[0]) !== -1) ? _arr[0] : "+852";
    ...
    if (typeof validation !== "function") { return; }
    _setIsValid(validation(_el.value));
    return;
  });

  const _onBlur = (e) => {
    const _val = `${_zipCodeRef.current.value} ${_phoneRef.current.value}`;
    const _el = (ref) ? ref.current : _newRef.current;
    _el.value = _val;

    if (typeof onBlur === "function") { onBlur(_val); }
    if (typeof validation !== "function") { return; }
    _setIsValid(validation(_val));
  };
  ...
  const _onChange = (e) => {
    ...
    const _el = (ref) ? ref.current : _newRef.current;
    _el.value = _val;
    if (typeof onChange === "function") { onChange(_val); }
  };

  const _label = (_isValid === true) ? (!label) ? false : placeholder : (_isValid instanceof Error) ? _isValid.message : _isValid.toString();
  const _placeholder = placeholder;
  const _baseInputStyle = { color, backgroundColor: "", border: "", outline: "", appearance: false };
  const _status = (_isValid === true) ? ((disabled) ? "disabled" : (eventName === "focus") ? "focus" : "normal") : "error";
  const _border = (underline !== false) ? ["", "", underline, ""] : border;
  const _isLabel = (() => {
    if (!floatLabel) { return true; }
  ...
  return (
    <InputBorder
      ...
      floatLabel={_isLabel}
    >
      <input ref={(ref) ? ref : _newRef} type="hidden" value={value} {...props}></input>
      <PureComponent
        ref={_zipCodeRef}
        tagname="select"
        disabled={disabled}
        baseStyle = {{ ..._baseInputStyle, size: "25%", ...baseStyle }}
        statusStyle={(disabled) ? { cursor: "not-allowed", ...statusStyle } : statusStyle }
        placeholder={((label && !floatLabel && !(_isValid instanceof Error)) || eventName === "focus") ? "" : _placeholder}
        onChange={_onChange}
      >
      {_dialCodes.map((d, idx) => <PureComponent key={idx} tagname="option" value={d}>{d}</PureComponent>)}
      </PureComponent>
      <PureComponent
        ref={_phoneRef}
        tagname="input"
        disabled={disabled}
        baseStyle = {{ ..._baseInputStyle, size: "75%", ...baseStyle }}
        statusStyle={(disabled) ? { cursor: "not-allowed", ...statusStyle } : statusStyle }
        placeholder={((label && !floatLabel && !(_isValid instanceof Error)) || eventName === "focus") ? "" : _placeholder}
        onBlur={_onBlur}
        onChange={_onChange}
...
_TelInput.displayName = "TelInput";
export const TelInput = _TelInput;
...
const _Select = React.forwardRef(({
  ...
  const _baseInputStyle = { color, backgroundColor: "", border: "", outline: "", size: "100%", ...baseStyle };
    ...
    >
      {
        ...
        >
        {
          _datalist.map((opt, idx) =>
            (typeof opt === "string") ?
              <PureComponent key={idx} tagname="option" value={opt}>{opt}</PureComponent> :
                (opt instanceof Object && opt !== null) ? 
                <PureComponent key={idx} tagname="option" value={opt.value}>{opt.label}</PureComponent>: null)
        }
        </PureComponent>
      }
      {children}
...
const _Toggle = React.forwardRef(({
  ...
  color = false, // false or "" or C() or M() based on media
  size = 50, // number or M() based on media
  ...
  return (
    ...
    >
      ...
      <PureComponent
        ...
        statusStyle={(checked) ? { backgroundColor: color, ...statusStyle } : {}}
      ...
      <PureComponent
        ...
        baseStyle={{
          ...
          width: size / 2 - _padding,
          left: _padding,
          top: 2 * _padding, // w : h = 2 : 1
          bottom: 2 * _padding,
          rounded: (rounded === false) ? "[]" : (rounded === true) ? "()" : rounded
        }}
        statusStyle={(checked) ? { translate: [1, 0] } : {}}
        transitions={{ all: { duration: 0.4, timeFunction: "ease-out", delay: 0 } }}
      />
      {children}
    </PureComponent>
